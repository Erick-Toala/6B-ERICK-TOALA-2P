...
const createEjercicioRealizado= async (req, res = response)=>{
    ...
    const existEjercicioRealizado =  await EjercicioRealizado.findOne({nombre: body.nombre})

    if (existEjercicioRealizado)
    {
        return res.status(400).json({
            msg:`El createEjercicioRealizado ${ existEjercicioRealizado.nombre } ya existe`
        })
    }
